...
export class LightService {
    ...
    Init() {
        ...
        this.hueUserToken = 'haKfNfJfRQqyRvdHO-2ub-u2Cp9jGKI7nExNquM1';
        this.baseUrl='http://192.168.178.56/api/haKfNfJfRQqyRvdHO-2ub-u2Cp9jGKI7nExNquM1/';
    ...
    notify(): Observable<ISensor[]> {
        let requestData={
            'effect':'colorloop','bri':80,'alert':'select'
        };

        let headers = new Headers({ 'Content-Type': 'application/json' });
        let options = new RequestOptions({ headers: headers });

        console.log(this.baseUrl+ "lights/")
        return this.http.put(this.baseUrl+ "lights/3/state", requestData, options)
            .map(this.extractData)
            .do(data => console.log('notify: ' + JSON.stringify(data)))
    ...
    getSensorsArray(): Observable<ISensor[]> {
        ...
        return this.http.get(`http://192.168.178.56/api/${this.hueUserToken}/`+ "sensors/")
            .map(this.extractArray)
            .do(data => console.log('getSensorsArray: ' + JSON.stringify(data)))
            .catch(this.handleError);
    }
